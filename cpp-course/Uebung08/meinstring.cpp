...
const MeinString& MeinString::operator+=(const MeinString& string) {
     insert(this->len, string);
     return *this;
}

char& MeinString::operator[](size_t index) { 
   return at(index); 
}

const char& MeinString::operator[](size_t index) const { 
   return at(index); 
}
...
MeinString operator+(const MeinString& a, const MeinString& b) {
   return MeinString(a) += b;
}

std::ostream& operator<<(std::ostream& os, const MeinString& string) {
   os << string.c_str();
   return os;
}